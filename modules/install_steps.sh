...
install_kernel() {
  if [ "${kernel_image}" = "none" ]; then
    ...
    #spawn_chroot 'DEBIAN_FRONTEND=noninteractive apt-get -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install ${kernel_image}' ||
     die "could not emerge kernel sources"
  fi
}
